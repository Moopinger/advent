...
func getEngineParts(input_row string, row_size int64, row_number int64, upper_row string, lower_row string) []Part {
	...
	for x_coord < row_size {
		...
		if isPointDigit(string(input_row[x_coord])) == true {
			...
			for index, char := range top_pattern {
				if string(char) == "*" {
					gear_x_coord := 0
					...
					if x_coord == 0 {
						gear_x_coord = int(x_coord) + index
					} else {
						gear_x_coord = int(x_coord-1) + index
					}
					gearid = strconv.Itoa(gear_x_coord) + strconv.Itoa(int(row_number)-1)
					break
				}
			}

			for index, char := range lower_pattern {
				if string(char) == "*" {
					gear_x_coord := 0
			...
			if left_char == "*" {
				gear = true
				gear_x_coord := int(x_coord) - 1
				gearid = strconv.Itoa(gear_x_coord) + strconv.Itoa(int(row_number))
			}

			if right_char == "*" {
				gear = true
				gear_x_coord := int(x_coord) + int(engine_length)
				gearid = strconv.Itoa(gear_x_coord) + strconv.Itoa(int(row_number))
			}
			pattern := left_char + " || " + top_pattern + " || " + right_char + " || " + lower_pattern
